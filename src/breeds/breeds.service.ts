...
export class BreedsService {
    ...
    async create(dto:CreateBreedDto){ // verificar se existe o specie id
        const specie = await this.specieRepository.findOne({where:{id:dto.specieId}});
        if(!specie) throw new NotFoundException('Specie not found');
        const breed = this.breedRepository.create({ name: dto.name, specie: specie});
    ...
    async update(id: number, dto: UpdateBreedDto) {
        const breed = await this.breedRepository.findOne({ where: { id }, relations: ['specie'] });
        if (!breed) throw new NotFoundException('Breed not found');
